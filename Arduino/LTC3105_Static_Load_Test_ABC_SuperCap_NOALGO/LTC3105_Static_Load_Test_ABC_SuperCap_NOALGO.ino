...
unsigned long lastMpptUpdateC = 0;     // Last time MPPT was updated
const unsigned long MPPT_UPDATE_INTERVAL = 100;  // Update every 1000ms (1 second)
...
void updateMPPT_C() {
  ...
  // Check if it's time to update MPPT (every 1 second)
  unsigned long currentTime = millis();
  if (currentTime - lastMpptUpdateC < MPPT_UPDATE_INTERVAL) {
    return;
  }
