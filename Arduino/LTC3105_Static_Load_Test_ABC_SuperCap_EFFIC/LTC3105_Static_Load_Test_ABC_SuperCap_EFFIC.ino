...
// Hill-climbing MPPT variables for Channel C (Efficiency Optimization)
float previousEfficiencyC = 0.0;      // Previous efficiency for comparison
...
void loop() {
  ...
  if (isConversionReady(INA219_ADDR_C2)) {
    ...
    // ===== CHANNEL C HILL-CLIMBING MPPT (EFFICIENCY OPTIMIZATION) =====
    updateMPPT_C();
  }
}
...
/* Hill-climbing MPPT algorithm for Channel C (Efficiency Optimization) */
void updateMPPT_C() {
  ...
  // Only run MPPT if we have valid power measurements to calculate efficiency
  if (powerInC <= 0) {
    return;
  }
  ...
  if (!mpptInitializedC) {
    previousEfficiencyC = efficiencyC;
    mpptInitializedC = true;
    return;
  }
  
  // Calculate efficiency change
  float efficiencyChange = efficiencyC - previousEfficiencyC;
  
  // Hill-climbing algorithm for efficiency
  if (efficiencyChange > 0) {
    ...
    // Efficiency unchanged, try a small perturbation in current direction
