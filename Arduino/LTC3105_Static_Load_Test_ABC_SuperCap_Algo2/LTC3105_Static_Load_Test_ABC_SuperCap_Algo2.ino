...
const float MPPC_MAX_VOLTAGE_A = 5.0;         // Upper clamp for Channel A MPPC voltage
...
const float MPPC_MAX_VOLTAGE_C = 2.0;         // Upper clamp for Channel C MPPC voltage
...
/* Improved Channel B Hill Climbing Algorithm */
void hillClimbMPPC_B() {
  // Calculate maximum allowed MPPC value (sourceVoltage - margin)
  float maxAllowedMPPC = currentSourceVoltageB - MPPC_VOLTAGE_MARGIN_B;
  
  // Wait for enough history before making decisions
  if (avgEfficiencyB == 0.0) {
    lastAvgEfficiencyB = avgEfficiencyB;
    return;
  }
  
  // Track best efficiency and corresponding MPPC value
  if (avgEfficiencyB > bestEfficiencyB) {
    bestEfficiencyB = avgEfficiencyB;
    bestMppcValueB = mppcValueB;
  }
  
  // Calculate efficiency improvement
  float efficiencyChange = avgEfficiencyB - lastAvgEfficiencyB;
...
/* Improved Channel C Hill Climbing Algorithm */
void hillClimbMPPC_C() {
  // Calculate maximum allowed MPPC value (sourceVoltage - margin)
  float maxAllowedMPPC = currentSourceVoltageC - MPPC_VOLTAGE_MARGIN_C;
  
  // Wait for enough history before making decisions
  if (avgEfficiencyC == 0.0) {
    lastAvgEfficiencyC = avgEfficiencyC;
    return;
  }
  
  // Track best efficiency and corresponding MPPC value
  if (avgEfficiencyC > bestEfficiencyC) {
    bestEfficiencyC = avgEfficiencyC;
    bestMppcValueC = mppcValueC;
  }
  
  // Calculate efficiency improvement
  float efficiencyChange = avgEfficiencyC - lastAvgEfficiencyC;
