...
const int DIGITAL_OUTPUT_PIN_A = 11; // Digital output pin for Schmitt Trigger A
const int DIGITAL_OUTPUT_PIN_B = 13; // Digital output pin for Schmitt Trigger B
const int DIGITAL_OUTPUT_PIN_C = 12; // Digital output pin for Schmitt Trigger C

// Schmitt Trigger A Parameters (measuring INA A2 bus voltage)
const float SCHMITT_UPPER_THRESHOLD_A = 5.25; // Upper threshold voltage
const float SCHMITT_LOWER_THRESHOLD_A = 4.75; // Lower threshold voltage
const float SCHMITT_HYSTERESIS_A = 0;         // Hysteresis value
bool schmittStateA = false;                    // Current state of Schmitt trigger A output

// Schmitt Trigger B Parameters (measuring INA B2 bus voltage)
const float SCHMITT_UPPER_THRESHOLD_B = 4.5;  // Upper threshold voltage
const float SCHMITT_LOWER_THRESHOLD_B = 2.5;  // Lower threshold voltage
const float SCHMITT_HYSTERESIS_B = 0;         // Hysteresis value
bool schmittStateB = false;                    // Current state of Schmitt trigger B output

// Schmitt Trigger C Parameters (measuring INA C2 bus voltage)
const float SCHMITT_UPPER_THRESHOLD_C = 4.5;  // Upper threshold voltage
const float SCHMITT_LOWER_THRESHOLD_C = 2.5;  // Lower threshold voltage
const float SCHMITT_HYSTERESIS_C = 0;         // Hysteresis value
bool schmittStateC = false;                    // Current state of Schmitt trigger C output
...
void setup() {
  ...
  digitalWrite(DIGITAL_OUTPUT_PIN_A, LOW);  // Initialize Schmitt Trigger A output to LOW
  pinMode(DIGITAL_OUTPUT_PIN_B, OUTPUT);
  digitalWrite(DIGITAL_OUTPUT_PIN_B, LOW);  // Initialize Schmitt Trigger B output to LOW
  pinMode(DIGITAL_OUTPUT_PIN_C, OUTPUT);
  digitalWrite(DIGITAL_OUTPUT_PIN_C, LOW);  // Initialize Schmitt Trigger C output to LOW
...
void loop() {
  ...
  if (isConversionReady(INA219_ADDR_A2)) {
    ...
    // ===== SCHMITT TRIGGER A - INA A2 BUS VOLTAGE =====
    if (schmittStateA == false) {
      // Currently LOW, check if we should go HIGH
      if (boostVoltageA >= (SCHMITT_UPPER_THRESHOLD_A - SCHMITT_HYSTERESIS_A)) {
        schmittStateA = true;
        digitalWrite(DIGITAL_OUTPUT_PIN_A, HIGH);
      }
    } else {
      // Currently HIGH, check if we should go LOW  
      if (boostVoltageA <= (SCHMITT_LOWER_THRESHOLD_A + SCHMITT_HYSTERESIS_A)) {
        schmittStateA = false;
        digitalWrite(DIGITAL_OUTPUT_PIN_A, LOW);
      }
    }
    Serial.print("schmitt state of A");
    Serial.println(schmittStateA);
  ...
  if (isConversionReady(INA219_ADDR_B2)) {
    ...
    // ===== SCHMITT TRIGGER B - INA B2 BUS VOLTAGE =====
    if (schmittStateB == false) {
      // Currently LOW, check if we should go HIGH
      if (boostVoltageB >= (SCHMITT_UPPER_THRESHOLD_B - SCHMITT_HYSTERESIS_B)) {
        schmittStateB = true;
        digitalWrite(DIGITAL_OUTPUT_PIN_B, HIGH);
      }
    } else {
      // Currently HIGH, check if we should go LOW  
      if (boostVoltageB <= (SCHMITT_LOWER_THRESHOLD_B + SCHMITT_HYSTERESIS_B)) {
        schmittStateB = false;
        digitalWrite(DIGITAL_OUTPUT_PIN_B, LOW);
      }
    }
  ...
  if (isConversionReady(INA219_ADDR_C2)) {
    ...
    // ===== SCHMITT TRIGGER C - INA C2 BUS VOLTAGE =====
    if (schmittStateC == false) {
      // Currently LOW, check if we should go HIGH
      if (boostVoltageC >= (SCHMITT_UPPER_THRESHOLD_C - SCHMITT_HYSTERESIS_C)) {
        schmittStateC = true;
        digitalWrite(DIGITAL_OUTPUT_PIN_C, HIGH);
      }
    } else {
      // Currently HIGH, check if we should go LOW  
      if (boostVoltageC <= (SCHMITT_LOWER_THRESHOLD_C + SCHMITT_HYSTERESIS_C)) {
        schmittStateC = false;
        digitalWrite(DIGITAL_OUTPUT_PIN_C, LOW);
      }
    }
