...
// Global variables for Channel D
int mppcValueD = 128;   // Default to middle value
float powerInD = 0.0;
float powerOutD = 0.0;
float efficiencyD = 0.0;
float currentSourceVoltageD = 0.0;
bool mppcEnabledD = false;

// Global variables for Channel E
int mppcValueE = 128;   // Default to middle value
float powerInE = 0.0;
float powerOutE = 0.0;
float efficiencyE = 0.0;
float currentSourceVoltageE = 0.0;
bool mppcEnabledE = false;

// Hill-climbing MPPT variables for Channel A (target: efficiency)
float previousEfficiencyA = 0.0;          // Previous efficiency for comparison
int mpptStepSizeA = 5;                     // Step size for MPPT adjustment
int mpptDirectionA = 1;                    // Direction of last step (1 or -1)
bool mpptInitializedA = false;             // Flag to track if MPPT has been initialized

// Hill-climbing MPPT variables for Channel B (target: output power)
...
// Hill-climbing MPPT variables for Channel C (target: output power)
...
// Hill-climbing MPPT variables for Channel D (target: output power)
float previousPowerOutD = 0.0;         // Previous power output for comparison
int mpptStepSizeD = 5;                 // Step size for MPPT adjustment
int mpptDirectionD = 1;                // Direction of last step (1 or -1)
bool mpptInitializedD = false;         // Flag to track if MPPT has been initialized

// Hill-climbing MPPT variables for Channel E (target: output power)
float previousPowerOutE = 0.0;         // Previous power output for comparison
int mpptStepSizeE = 5;                 // Step size for MPPT adjustment
int mpptDirectionE = 1;                // Direction of last step (1 or -1)
bool mpptInitializedE = false;         // Flag to track if MPPT has been initialized
...
void setMPPC_D(int potValue);
void setMPPC_E(int potValue);
void updateMPPT_A();  // New function for Channel A efficiency-optimized MPPT
void updateMPPT_B();  // New function for Channel B MPPT
void updateMPPT_C();  // New function for Channel C MPPT
void updateMPPT_D();  // New function for Channel D MPPT
void updateMPPT_E();  // New function for Channel E MPPT
...
void setup() {
  ...
  digitalWrite(BOOST_PIN, LOW);  // Initialize boost pin to LOW
  pinMode(CHARGE_PIN, OUTPUT);
  digitalWrite(CHARGE_PIN, LOW);  // Initialize charge pin to LOW
  pinMode(SHDN_0_PIN, OUTPUT);
  digitalWrite(SHDN_0_PIN, HIGH);  // Initialize shutdown pin to HIGH (enabled)
  pinMode(SHDN_1_PIN, OUTPUT);
  digitalWrite(SHDN_1_PIN, HIGH);  // Initialize shutdown pin to HIGH (enabled)
  pinMode(SHDN_2_PIN, OUTPUT);
  digitalWrite(SHDN_2_PIN, HIGH);  // Initialize shutdown pin to HIGH (enabled)
  pinMode(SHDN_3_PIN, OUTPUT);
  digitalWrite(SHDN_3_PIN, HIGH);  // Initialize shutdown pin to HIGH (enabled)
  ...
  // Initialize Channel D AD5280 digital potentiometer
  bool adConnectedD = AD01_D.begin();
  Serial.print(F("Channel D AD5280 connected: "));
  Serial.println(adConnectedD ? F("true") : F("false"));
  // Initialize Channel E AD5280 digital potentiometer
  bool adConnectedE = AD01_E.begin();
  Serial.print(F("Channel E AD5280 connected: "));
  Serial.println(adConnectedE ? F("true") : F("false"));
  ...
  // Configure Channel D INA219 sensors
  writeReg16(INA219_ADDR_D1, 0x00, INA219_CONFIG);      // Config register
  writeReg16(INA219_ADDR_D1, 0x05, INA219_CALIBRATION); // Calibration register
  writeReg16(INA219_ADDR_D2, 0x00, INA219_CONFIG);      // Config register
  writeReg16(INA219_ADDR_D2, 0x05, INA219_CALIBRATION); // Calibration register
  // Configure Channel E INA219 sensors
  writeReg16(INA219_ADDR_E1, 0x00, INA219_CONFIG);      // Config register
  writeReg16(INA219_ADDR_E1, 0x05, INA219_CALIBRATION); // Calibration register
  writeReg16(INA219_ADDR_E2, 0x00, INA219_CONFIG);      // Config register
  writeReg16(INA219_ADDR_E2, 0x05, INA219_CALIBRATION); // Calibration register
...
void loop() {
  ...
  if (isConversionReady(INA219_ADDR_A1)) {
    ...
    if (currentSourceVoltageA >= 0.3) {
      ...
      if (mppcEnabledA) {  // Only reset if transitioning from enabled to disabled
        ...
        mpptInitializedA = false;  // Reset MPPT initialization
      }
      mppcEnabledA = false;
    }
  ...
  if (isConversionReady(INA219_ADDR_A2)) {
    ...
    // ===== CHANNEL A EFFICIENCY-OPTIMIZED MPPT =====
    updateMPPT_A();
...
/* Set Channel D MPPC by writing digital potentiometer value (0-255) */
void setMPPC_D(int potValue) {
  // Clamp input to valid range (0-255)
  if (potValue < 0) potValue = 0;
  if (potValue > 255) potValue = 255;
  
  // Write to Channel D digital potentiometer
  AD01_D.write(potValue);
}

/* Set Channel E MPPC by writing digital potentiometer value (0-255) */
void setMPPC_E(int potValue) {
  // Clamp input to valid range (0-255)
  if (potValue < 0) potValue = 0;
  if (potValue > 255) potValue = 255;
  
  // Write to Channel E digital potentiometer
  AD01_E.write(potValue);
}

/* Hill-climbing MPPT algorithm for Channel A - optimized for efficiency */
void updateMPPT_A() {
  ...
  // Need minimum power input to make efficiency calculation meaningful
  if (powerInA < 0.001) {  // Less than 1mW
    return;
  }
  
  // Initialize on first run
  if (!mpptInitializedA) {
    previousEfficiencyA = efficiencyA;
    mpptInitializedA = true;
    return;
  }
  
  // Calculate efficiency change
  float efficiencyChange = efficiencyA - previousEfficiencyA;
  
  // Hill-climbing algorithm for efficiency optimization
  if (efficiencyChange > 0.1) {  // Efficiency improved by more than 0.1%
    // Efficiency increased, continue in the same direction
    // Keep the same direction as last step
  } else if (efficiencyChange < -0.1) {  // Efficiency decreased by more than 0.1%
    // Efficiency decreased, reverse direction
    mpptDirectionA = -mpptDirectionA;
  } else {
    // Efficiency unchanged or small change, try a small perturbation in current direction
  ...
  if (newMppcValue < 0) {
    newMppcValue = 0;
    mpptDirectionA = 1;  // Reverse direction when hitting boundary
  ...
  // Store current efficiency for next iteration
  previousEfficiencyA = efficiencyA;
}
...
/* Hill-climbing MPPT algorithm for Channel D */
void updateMPPT_D() {
  // Only run MPPT if Channel D is enabled
  if (!mppcEnabledD) {
    return;
  }
  ...
  // Initialize on first run
  if (!mpptInitializedD) {
    previousPowerOutD = powerOutD;
    mpptInitializedD = true;
    return;
  }
  
  // Calculate power change
  float powerChange = powerOutD - previousPowerOutD;
  ...
  // Calculate new MPPC value
  int newMppcValue = mppcValueD + (mpptStepSizeD * mpptDirectionD);
  
  // Clamp to valid range (0-255)
  if (newMppcValue < 0) {
    newMppcValue = 0;
    mpptDirectionD = 1;  // Reverse direction when hitting boundary
  } else if (newMppcValue > 255) {
    newMppcValue = 255;
    mpptDirectionD = -1; // Reverse direction when hitting boundary
  }
  ...
  // Update MPPC value and apply it
  mppcValueD = newMppcValue;
  setMPPC_D(mppcValueD);
  
  // Store current power for next iteration
  previousPowerOutD = powerOutD;
}

/* Hill-climbing MPPT algorithm for Channel E */
void updateMPPT_E() {
  // Only run MPPT if Channel E is enabled
  if (!mppcEnabledE) {
    return;
  }
  ...
  // Initialize on first run
  if (!mpptInitializedE) {
    previousPowerOutE = powerOutE;
    mpptInitializedE = true;
    return;
  }
  
  // Calculate power change
  float powerChange = powerOutE - previousPowerOutE;
  ...
  // Calculate new MPPC value
  int newMppcValue = mppcValueE + (mpptStepSizeE * mpptDirectionE);
  
  // Clamp to valid range (0-255)
  if (newMppcValue < 0) {
    newMppcValue = 0;
    mpptDirectionE = 1;  // Reverse direction when hitting boundary
  } else if (newMppcValue > 255) {
    newMppcValue = 255;
    mpptDirectionE = -1; // Reverse direction when hitting boundary
  }
  ...
  // Update MPPC value and apply it
  mppcValueE = newMppcValue;
  setMPPC_E(mppcValueE);
