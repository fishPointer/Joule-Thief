...
void hillClimbMPPC_A() {
  ...
  if (efficiencyChange > MIN_EFFICIENCY_IMPROVEMENT) {
    // Significant improvement - continue in same direction and possibly increase step size
    stepSizeA = min(stepSizeA * STEP_SIZE_INCREASE, MAX_STEP_SIZE);
    stagnationCounterA = 0;
  } else if (efficiencyChange < -MIN_EFFICIENCY_IMPROVEMENT) {
    // Significant decrease - reverse direction and reduce step size
    directionA *= -1;
    stepSizeA = max(stepSizeA * STEP_SIZE_DECAY, MIN_STEP_SIZE);
    stagnationCounterA = 0;
  } else {
    // Minimal change - increment stagnation counter
    stagnationCounterA++;
    
    // If stagnating, try a larger step or return to best known point
    if (stagnationCounterA >= STAGNATION_THRESHOLD) {
      if (abs(mppcValueA - bestMppcValueA) > 0.1) {
        ...
        // Try larger step in random direction
        directionA = (millis() % 2 == 0) ? 1 : -1;
        stepSizeA = min(stepSizeA * 1.5, MAX_STEP_SIZE);
        stagnationCounterA = 0;
      }
    }
  }
...
void hillClimbMPPC_B() {
  ...
  if (powerChange > minPowerImprovement) {
    // Significant improvement - continue in same direction and possibly increase step size
    stepSizeB = min(stepSizeB * STEP_SIZE_INCREASE, MAX_STEP_SIZE);
    stagnationCounterB = 0;
  } else if (powerChange < -minPowerImprovement) {
    // Significant decrease - reverse direction and reduce step size
    directionB *= -1;
    stepSizeB = max(stepSizeB * STEP_SIZE_DECAY, MIN_STEP_SIZE);
    stagnationCounterB = 0;
  } else {
    // Minimal change - increment stagnation counter
    stagnationCounterB++;
    ...
    if (stagnationCounterB >= STAGNATION_THRESHOLD) {
      if (abs(mppcValueB - bestMppcValueB) > 0.1) {
        // Return to best known point
        ...
        stepSizeB = min(stepSizeB * 1.5, MAX_STEP_SIZE);
        stagnationCounterB = 0;
      }
    }
  }
...
void hillClimbMPPC_C() {
  ...
  if (powerChange > minPowerImprovement) {
    // Significant improvement - continue in same direction and possibly increase step size
    stepSizeC = min(stepSizeC * STEP_SIZE_INCREASE, MAX_STEP_SIZE);
    stagnationCounterC = 0;
  } else if (powerChange < -minPowerImprovement) {
    // Significant decrease - reverse direction and reduce step size
    directionC *= -1;
    stepSizeC = max(stepSizeC * STEP_SIZE_DECAY, MIN_STEP_SIZE);
    stagnationCounterC = 0;
  } else {
    // Minimal change - increment stagnation counter
    stagnationCounterC++;
    ...
    if (stagnationCounterC >= STAGNATION_THRESHOLD) {
      if (abs(mppcValueC - bestMppcValueC) > 0.1) {
        // Return to best known point
        ...
        stepSizeC = min(stepSizeC * 1.5, MAX_STEP_SIZE);
        stagnationCounterC = 0;
      }
    }
  }
