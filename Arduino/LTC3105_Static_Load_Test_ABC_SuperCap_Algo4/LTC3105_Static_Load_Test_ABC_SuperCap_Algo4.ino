...
const int DIGITAL_OUTPUT_PIN_2 = 12;    // Second digital output pin for boost voltage monitoring


// Schmitt Trigger Parameters
const float SCHMITT_UPPER_THRESHOLD = 4.5;    // Upper threshold voltage
const float SCHMITT_LOWER_THRESHOLD = 2.0;    // Lower threshold voltage
...
const float SCHMITT_LOWER_THRESHOLD_2 = 4.75;    // Lower threshold voltage for boost
...
const float MAX_STEP_SIZE = 0.01;             // Maximum step size (limited to 0.01V)
...
float stepSizeA = 0.01;                       // Start with maximum allowed step size
int directionA = 1;
...
float powerOutHistoryB[EFFICIENCY_HISTORY_SIZE] = {0.0};
int powerOutIndexB = 0;
float avgPowerOutB = 0.0;
float lastAvgPowerOutB = 0.0;
float stepSizeB = 0.01;                       // Start with maximum allowed step size
int directionB = 1;
unsigned long lastAdjustmentB = 0;
float currentSourceVoltageB = 0.0;
bool mppcEnabledB = false;
int stagnationCounterB = 0;
float bestPowerOutB = 0.0;
...
float powerOutHistoryC[EFFICIENCY_HISTORY_SIZE] = {0.0};
int powerOutIndexC = 0;
float avgPowerOutC = 0.0;
float lastAvgPowerOutC = 0.0;
float stepSizeC = 0.01;                       // Start with maximum allowed step size
int directionC = 1;
unsigned long lastAdjustmentC = 0;
float currentSourceVoltageC = 0.0;
bool mppcEnabledC = false;
int stagnationCounterC = 0;
float bestPowerOutC = 0.0;
...
// MODIFIED: Changed adjustment interval from 250ms to 1000ms (1 second)
const unsigned long adjustmentInterval = 1000; // Adjust every 1000ms (1 second)
...
void loop() {
  ...
  if (isConversionReady(INA219_ADDR_A2)) {
    ...
    // MODIFIED: Hill climbing algorithm now runs every 1 second instead of 250ms
    ...
    // Schmitt trigger with bounds around 4.75V and 5.25V
    if (schmittState2 == false) {
      // Currently LOW, check if we should go HIGH
      if (boostVoltageA >= (SCHMITT_UPPER_THRESHOLD_2 - SCHMITT_HYSTERESIS_2)) {  // 5.25V
        schmittState2 = true;
        digitalWrite(DIGITAL_OUTPUT_PIN_2, HIGH);
      }
    } else {
      // Currently HIGH, check if we should go LOW  
      if (boostVoltageA <= (SCHMITT_LOWER_THRESHOLD_2 + SCHMITT_HYSTERESIS_2)) {  // 4.75V
        schmittState2 = false;
  ...
  if (isConversionReady(INA219_ADDR_B1)) {
    ...
    if (currentSourceVoltageB >= 0.3) {
      if (!mppcEnabledB) {
        ...
        // Clear power output history
        for (int i = 0; i < EFFICIENCY_HISTORY_SIZE; i++) {
          powerOutHistoryB[i] = 0.0;
        }
        powerOutIndexB = 0;
      }
      mppcEnabledB = true;
  ...
  if (isConversionReady(INA219_ADDR_B2)) {
    ...
    // Calculate efficiency and update power output history
    if (powerInB > 0) {
      efficiencyB = (powerOutB / powerInB) * 100.0;
      
      // Update power output history
      updateEfficiencyHistory(powerOutHistoryB, &powerOutIndexB, powerOutB);
      avgPowerOutB = calculateAverageEfficiency(powerOutHistoryB, EFFICIENCY_HISTORY_SIZE);
    }
    ...
    Serial.print(",avg_power_out_b=");
    Serial.print(avgPowerOutB * 1000, 6); // Convert to mW
    Serial.println();
  ...
  if (isConversionReady(INA219_ADDR_C2)) {
    ...
    if (powerInC > 0) {
      ...
      // Update power output history
      updateEfficiencyHistory(powerOutHistoryC, &powerOutIndexC, powerOutC);
      avgPowerOutC = calculateAverageEfficiency(powerOutHistoryC, EFFICIENCY_HISTORY_SIZE);
    }
    ...
    Serial.print(",avg_power_out_c=");
    Serial.print(avgPowerOutC * 1000, 6); // Convert to mW
    Serial.println();
...
void setMPPC_C(float targetVoltage) {
  // Clamp input to valid range using Channel C constants
  if (targetVoltage < MPPC_MIN_VOLTAGE_C) targetVoltage = MPPC_MIN_VOLTAGE_C;
  if (targetVoltage > MPPC_MAX_VOLTAGE_C) targetVoltage = MPPC_MAX_VOLTAGE_C;
  
  // Map 0-5.0V range to 0-256 range
  float mapped = targetVoltage * 256.0 / MPPC_MAX_VOLTAGE_C;
  int potValue = (int)mapped;
...
/* Improved Channel A Hill Climbing Algorithm */
void hillClimbMPPC_A() {
  // Calculate maximum allowed MPPC value (sourceVoltage - margin)
  float maxAllowedMPPC = currentSourceVoltageA - MPPC_VOLTAGE_MARGIN_A;
  
  // Wait for enough history before making decisions
  if (avgEfficiencyA == 0.0) {
    lastAvgEfficiencyA = avgEfficiencyA;
    return;
  }
  
  // Track best efficiency and corresponding MPPC value
  if (avgEfficiencyA > bestEfficiencyA) {
    bestEfficiencyA = avgEfficiencyA;
    bestMppcValueA = mppcValueA;
  }
  
  // Calculate efficiency improvement
  float efficiencyChange = avgEfficiencyA - lastAvgEfficiencyA;
  
  // Decision logic
  if (efficiencyChange > MIN_EFFICIENCY_IMPROVEMENT) {
    // Significant improvement - continue in same direction, keep step size at maximum
    stepSizeA = MAX_STEP_SIZE;  // Always use maximum allowed step size (0.01V)
    stagnationCounterA = 0;
  } else if (efficiencyChange < -MIN_EFFICIENCY_IMPROVEMENT) {
    // Significant decrease - reverse direction, keep step size at maximum
    directionA *= -1;
    stepSizeA = MAX_STEP_SIZE;  // Always use maximum allowed step size (0.01V)
    stagnationCounterA = 0;
  } else {
    // Minimal change - increment stagnation counter
    stagnationCounterA++;
    
    // If stagnating, try returning to best known point or change direction
    if (stagnationCounterA >= STAGNATION_THRESHOLD) {
      if (abs(mppcValueA - bestMppcValueA) > 0.01) {
        // Return to best known point
        mppcValueA = bestMppcValueA;
        setMPPC_A(mppcValueA);
        stagnationCounterA = 0;
        lastAvgEfficiencyA = avgEfficiencyA;
        return;
  ...
  // Apply the step (limited to 0.01V maximum)
  mppcValueA += directionA * stepSizeA;
  
  // Constrain mppcValue to valid range with voltage constraint
  mppcValueA = constrain(mppcValueA, MPPC_MIN_VOLTAGE_A, min(maxAllowedMPPC, MPPC_MAX_VOLTAGE_A));
  
  // If we hit the voltage constraint, reverse direction for next iteration
  if (mppcValueA >= maxAllowedMPPC || mppcValueA <= MPPC_MIN_VOLTAGE_A) {
    directionA *= -1;
  }
...
/* Power-Optimized Channel B Hill Climbing Algorithm */
void hillClimbMPPC_B() {
  // Calculate maximum allowed MPPC value (sourceVoltage - margin)
  float maxAllowedMPPC = currentSourceVoltageB - MPPC_VOLTAGE_MARGIN_B;
  
  // Wait for enough history before making decisions
  if (avgPowerOutB == 0.0) {
    lastAvgPowerOutB = avgPowerOutB;
    return;
  }
  
  // Track best power output and corresponding MPPC value
  if (avgPowerOutB > bestPowerOutB) {
    bestPowerOutB = avgPowerOutB;
    bestMppcValueB = mppcValueB;
  }
  
  // Calculate power output change (in watts)
  float powerChange = avgPowerOutB - lastAvgPowerOutB;
  // Convert minimum improvement threshold from percentage to absolute power
  float minPowerImprovement = bestPowerOutB * (MIN_EFFICIENCY_IMPROVEMENT / 100.0);
  
  // Decision logic based on power output change
  if (powerChange > minPowerImprovement) {
    // Significant improvement - continue in same direction, keep step size at maximum
    stepSizeB = MAX_STEP_SIZE;  // Always use maximum allowed step size (0.01V)
    stagnationCounterB = 0;
  } else if (powerChange < -minPowerImprovement) {
    // Significant decrease - reverse direction, keep step size at maximum
    directionB *= -1;
    stepSizeB = MAX_STEP_SIZE;  // Always use maximum allowed step size (0.01V)
    stagnationCounterB = 0;
    ...
    // If stagnating, try returning to best known point or change direction
    if (stagnationCounterB >= STAGNATION_THRESHOLD) {
      if (abs(mppcValueB - bestMppcValueB) > 0.01) {
        // Return to best known point
        mppcValueB = bestMppcValueB;
        setMPPC_B(mppcValueB);
        stagnationCounterB = 0;
        lastAvgPowerOutB = avgPowerOutB;
        return;
  ...
  // Constrain mppcValue to valid range with voltage constraint
  mppcValueB = constrain(mppcValueB, MPPC_MIN_VOLTAGE_B, min(maxAllowedMPPC, MPPC_MAX_VOLTAGE_B));
  
  // If we hit the voltage constraint, reverse direction for next iteration
  if (mppcValueB >= maxAllowedMPPC || mppcValueB <= MPPC_MIN_VOLTAGE_B) {
    directionB *= -1;
  }
  ...
  // Store current power output for next comparison
  lastAvgPowerOutB = avgPowerOutB;
}

/* Power-Optimized Channel C Hill Climbing Algorithm */
void hillClimbMPPC_C() {
  // Calculate maximum allowed MPPC value (sourceVoltage - margin)
  float maxAllowedMPPC = currentSourceVoltageC - MPPC_VOLTAGE_MARGIN_C;
  
  // Wait for enough history before making decisions
  if (avgPowerOutC == 0.0) {
    lastAvgPowerOutC = avgPowerOutC;
    return;
  }
  
  // Track best power output and corresponding MPPC value
  if (avgPowerOutC > bestPowerOutC) {
    bestPowerOutC = avgPowerOutC;
    bestMppcValueC = mppcValueC;
  }
  
  // Calculate power output change (in watts)
  float powerChange = avgPowerOutC - lastAvgPowerOutC;
  // Convert minimum improvement threshold from percentage to absolute power
  float minPowerImprovement = bestPowerOutC * (MIN_EFFICIENCY_IMPROVEMENT / 100.0);
  
  // Decision logic based on power output change
  if (powerChange > minPowerImprovement) {
    // Significant improvement - continue in same direction, keep step size at maximum
    stepSizeC = MAX_STEP_SIZE;  // Always use maximum allowed step size (0.01V)
    stagnationCounterC = 0;
  } else if (powerChange < -minPowerImprovement) {
    // Significant decrease - reverse direction, keep step size at maximum
    directionC *= -1;
    ...
    // If stagnating, try returning to best known point or change direction
    if (stagnationCounterC >= STAGNATION_THRESHOLD) {
      if (abs(mppcValueC - bestMppcValueC) > 0.01) {
        // Return to best known point
        mppcValueC = bestMppcValueC;
        setMPPC_C(mppcValueC);
        stagnationCounterC = 0;
        lastAvgPowerOutC = avgPowerOutC;
        return;
  ...
  // Apply the step (limited to 0.01V maximum)
  mppcValueC += directionC * stepSizeC;
  
  // Constrain mppcValue to valid range with voltage constraint
  mppcValueC = constrain(mppcValueC, MPPC_MIN_VOLTAGE_C, min(maxAllowedMPPC, MPPC_MAX_VOLTAGE_C));
  
  // If we hit the voltage constraint, reverse direction for next iteration
  if (mppcValueC >= maxAllowedMPPC || mppcValueC <= MPPC_MIN_VOLTAGE_C) {
    directionC *= -1;
  }
